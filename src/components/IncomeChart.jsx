...
const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
        ...
        return (
            <div className="bg-white p-3.5 border border-emerald-100 shadow-xl rounded-2xl min-w-[150px] animate-in">
...
const EarningsChart = ({ earnings }) => {
    const chartData = useMemo(() => {
        ...
        const aggregated = earnings.reduce((acc, current) => {
            ...
            acc[dateStr].details.push({ name: current.name || current.categoryName || "Earning", amount: currentAmount });
            return acc;
        }, {});
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-gray-400 mt-0.5">Cumulative trend across {chartData.length} day{chartData.length !== 1 ? 's' : ''}</p>
                    </div>
                </div>
                {/* Mini KPI Badges */}
                <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {peakDay && (
                        <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-teal-700 bg-teal-50 px-3 py-1.5 rounded-lg border border-teal-100">
                            <Calendar size={11} />
                            Peak: {peakDay.date}
                        </span>
                    )}
                ...
                <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={chartData} margin={{ top: 8, right: 8, left: -10, bottom: 0 }}>
                        ...
                        <Tooltip
                            ...
                            cursor={{ stroke: '#10b981', strokeWidth: 1.5, strokeDasharray: '5 5' }}
                        ...
                        <Area
                            ...
                            activeDot={{ r: 6, fill: '#10b981', stroke: '#fff', strokeWidth: 3, filter: 'drop-shadow(0 2px 6px rgba(16,185,129,0.5))' }}
                            dot={chartData.length <= 8 ? { r: 3, fill: '#10b981', stroke: '#fff', strokeWidth: 2 } : false}
